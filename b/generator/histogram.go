...
func NewHistogramFromFile(name string) *Histogram {
	...
	if err != nil {
		log.Fatalf("load histogram file %s failed %v", name, err)
	}

	lines := strings.Split(strings.TrimSpace(string(data)), "\n")
	line := strings.Split(strings.TrimSpace(lines[0]), "\t")
	if line[0] != "BlockSize" {
		log.Fatalf("First line of histogram is not the BlockSize but %s", line)
	}

	blockSize, err := strconv.ParseInt(line[1], 10, 64)
	if err != nil {
		log.Fatalf("parse BlockSize failed %v", err)
	}
