...
// ForYield applies a transformation function to each element of the input slice and returns a new slice.
func ForYield[T any, startIndex int, endIndex int, U any](slice []T, transform func(T) U) []U {
	var result []U
...
func MakePlots(start time.Time, end time.Time, path string) {
	var pi = []*plotInfo{
...
func initPlotInfo(start time.Time, end time.Time, m MetricSlice, title string, yAxis string, path string) *plotInfo {
	p := plot.New()
	p.Title.Text = title
...
func (m *metrics) plotConfig(start time.Time, end time.Time, filename string) {
	config := m.config
	...
	addLabel(img, leftIndent+40, 90, fmt.Sprintf("Duration: %d seconds", int(end.Sub(start).Seconds())), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 120, fmt.Sprintf("Target Requests per Second: %d", config.Performance.TargetOperationsPerSec.Value), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 150, fmt.Sprintf("Nodes: %d", len(config.Cache.Nodes)), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 180, fmt.Sprintf("Virtual Nodes: %d", config.Cache.VirtualNodes.Value), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 210, fmt.Sprintf("Read Percentage: %d%%", int(config.Workload.ReadProportion.Value*100.0)), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 240, fmt.Sprintf("Failures: %d", failures), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 270, fmt.Sprintf("Key Range: %d to %d", config.Workload.KeyRangeLowerBound.Value, config.Workload.KeyRangeLowerBound.Value+config.Performance.InsertCount.Value-1), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 300, fmt.Sprintf("Concurrency: %v", config.Performance.ThreadCount.Value), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 330, fmt.Sprintf("Warmup Time: %d", config.Measurements.WarmUpTime.Value), "fonts/roboto/Roboto-Medium.ttf", 16.0)
	addLabel(img, leftIndent+40, 360, fmt.Sprintf("Request Distribution: %d", config.Workload.RequestDistribution.Value), "fonts/roboto/Roboto-Medium.ttf", 16.0)
