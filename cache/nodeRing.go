...
//			// make a unique id for each virtual node
//			nodeID := fmt.Sprintf("%s", generateRandomString(10))
//			hash := util.StringHash(nodeID) // hash the unique node identifier
...
//func (nr *NodeRing) GetNodes(key string) []int {
//	hash := util.StringHash(key) // first, get the hash for the key
//
//	idx, ok := nr.memIdx[hash]
//	if !ok {
